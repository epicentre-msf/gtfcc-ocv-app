    ...
    div(
      ...
      div(
        ...
        shinyWidgets::pickerInput(
          inputId = ns("status"),
          label = "Status",
          choices = unique(df_request$request_status) %>% na.omit(),
          options = picker_opts(),
          multiple = TRUE
        ),
        shinyWidgets::pickerInput(
          inputId = ns("context"),
          label = "Context",
          choices = unique(df_request$context) %>% na.omit(),
          options = picker_opts(),
          multiple = TRUE
        ),
        shinyWidgets::pickerInput(
          ...
          label = "Mechanism",
          choices = unique(df_request$request_mechanism) %>% na.omit(),
          options = picker_opts(),
          multiple = TRUE
        ),
        shinyWidgets::pickerInput(
          ...
          choices = unique(df_request$request_agency) %>% na.omit(),
          options = picker_opts(),
          multiple = TRUE
        ),
        shinyWidgets::pickerInput(
        ...
        actionButton(ns("reset"), "Reset Inputs", icon = icon("refresh"), class = "btn-info btn-sm", style = "color: #fff;"),
        actionButton(ns("close"), "Close", icon = icon("times"), class = "btn-danger btn-sm", style = "color: #fff;")
      )
    )
  )
}
